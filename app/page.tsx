    ...
    const fetchAllData = async (signal?: AbortSignal) => {
      ...
      try {
        const [homeResponse, mixResponse] = await Promise.all([
          fetch('/api/home', { cache: 'no-store', signal }),
          fetch('/api/mix/list?page=1', { cache: 'no-store', signal }),
        ])

        if (!homeResponse.ok || !mixResponse.ok) {
        ...
        setLoadError('تعذر تحديث المحتوى الآن، يتم عرض آخر بيانات متاحة.')
      } finally {
        if (isMounted) {
  ...
  return (
    <div className="app-shell min-h-screen bg-[#0a0620]">
      ...
      {(isRefreshing || loadError) && (
        <div className="mx-auto flex max-w-[1540px] items-center justify-between gap-3 px-4 pt-4 text-sm sm:px-8 lg:px-10">
          <div className="rounded-full border border-white/10 bg-white/[0.04] px-4 py-2 text-white/70">
            {isRefreshing ? 'جارٍ تحديث محتوى الصفحة...' : 'تم تحميل المحتوى'}
          </div>
          {loadError ? (
            <div className="rounded-full border border-amber-300/25 bg-amber-300/10 px-4 py-2 text-amber-200">
              {loadError}
            </div>
          ) : null}
        </div>
