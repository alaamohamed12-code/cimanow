...
import { getShowsDetails, ShowsDetailsPayload } from '@/lib/fetchShowsDetails'
...
export default async function ShowsDetailsPage({ params }: PageProps) {
  ...
  const showsPath = `/shows/${slug.join('/')}`
  const originalUrl = `https://ak.sv${showsPath}`

  let data: ShowsDetailsPayload | null = null
  try {
    data = await getShowsDetails(showsPath)
  } catch {
    // fallback UI
  }
    ...
    return (
      <div className="app-shell min-h-screen flex items-center justify-center">
        <div className="section-shell p-8 max-w-lg mx-auto text-center">
          ...
          <h1 className="text-2xl font-bold gradient-text mb-3">تعذّر تحميل تفاصيل البرنامج</h1>
          <p className="text-gray-400 mb-6 text-sm leading-7">
            لم نتمكن من جلب بيانات هذا البرنامج. يمكنك مشاهدته مباشرةً من الموقع الأصلي.
          </p>
          <a
            href={originalUrl}
            target="_blank"
          ...
          >
            مشاهدة البرنامج على الموقع الأصلي ↗
          </a>
          <div className="mt-4">
  ...
  const episodesSection = data.hasEpisodes && data.episodes.length > 0 ? (
