...
const stripTitlePrefix = (value: string): string =>
  value.replace(/^\s*(فيلم|مسلسل|برنامج|الحلقة)\s+/u, '').trim()
...
function WatchExperience({
    ...
    fetch(`/api/watch?url=${encodeURIComponent(url)}`, { signal: controller.signal })
      .then(async (response) => {
        ...
        if (!response.ok || payload.error) {
          throw new Error(payload.error || 'تعذر تحميل بيانات المشاهدة.')
        }
        setData(payload)
      })
      .catch((error: Error) => {
  ...
  const descriptionText =
    preDescription || 'صفحة مشاهدة متكاملة مع عداد قصير قبل بدء التشغيل التلقائي وروابط الجودة المتاحة.'
        ...
        <div className="rounded-[26px] border border-white/12 bg-[linear-gradient(180deg,rgba(17,17,30,0.92),rgba(10,10,18,0.72))] p-5 shadow-[0_22px_54px_rgba(0,0,0,0.35)] sm:p-6">
          {pageError ? (
            <div className="rounded-2xl border border-rose-400/35 bg-rose-500/10 p-5 text-center space-y-3">
              <p className="text-rose-200">تعذر تحميل بيانات المشغل</p>
              {url ? (
                <a
                  href={url}
                  target="_blank"
            ...
            <div className="flex min-h-[420px] flex-col items-center justify-center gap-4 rounded-[22px] border border-white/10 bg-[#121318]/90">
              ...
              <p className="text-white/55">جارٍ تجهيز المشغل...</p>
            </div>
          ) : (
                  ...
                  {currentVideoSrc ? (
                    <>
                      ...
                      >
                        ...
                        <p className="p-4 text-gray-300">متصفحك لا يدعم مشغل الفيديو.</p>
                      </video>
                  ...
                  {data.qualities.map((quality, idx) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="text-sm font-bold text-white">{quality.size || 'حجم غير محدد'}</div>
                      </button>
                    )
                  })}
                </div>
