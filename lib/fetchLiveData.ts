...
import { getStaticFeatured, getStaticMovies, getStaticSeries, getStaticShows } from '@/lib/staticCatalog'
...
const enrichWithLiveImages = async (section: SectionName): Promise<Content[]> => {
  if (process.env.NODE_ENV === 'production') {
    if (section === 'featured') return getStaticFeatured()
    if (section === 'movies') return getStaticMovies()
    if (section === 'series') return getStaticSeries()
    return getStaticShows()
  }
