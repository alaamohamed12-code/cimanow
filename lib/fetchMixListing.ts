...
const PAGE_RE = /(?:https?:\/\/ak\.sv)?\/mix\?[^"'\s>]*page=(\d+)/gi
...
export interface MixListingResponse {
  items: Content[]
  page: number
  totalPages: number
  filterFields: Array<{
...
const extractGenres = (text: string, title: string): string => {
  const merged = `${title} ${text}`
  const matches = GENRE_TOKENS.filter((token) => merged.includes(token))
  return matches.length > 0 ? Array.from(new Set(matches)).join('، ') : 'منوعات'
}

const extractDescription = (title: string, metadata: string): string => {
  const compact = metadata.replace(SPACE_RE, ' ').trim()
  if (!compact) {
    return `${title} - عنصر منوعات من القائمة الرئيسية`
  }

  return `${title} - ${compact.slice(0, 120)}`
}
...
const extractFilterFields = (html: string) => {
  const fields: MixListingResponse['filterFields'] = []
...
export const getMixListing = async (
  page: number,
  sourceFilters: SourceFilters = {},
  search?: string
): Promise<MixListingResponse> => {
  ...
  const html = await fetchHtml(`https://ak.sv/mix?${params.toString()}`)
  ...
  const items = titleMatches.map((entry, index) => {
    ...
    const genre = extractGenres(cleanChunk, entry.title)
  ...
  const filterFields = extractFilterFields(html)
