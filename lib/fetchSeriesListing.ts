...
const PAGE_RE = /(?:https?:\/\/ak\.sv)?\/series\?[^"'\s>]*page=(\d+)/gi
...
export interface SeriesListingResponse {
  items: Content[]
  page: number
  totalPages: number
  filterFields: Array<{
...
const extractDescription = (title: string, metadata: string): string => {
  ...
  if (!compact) {
    return `${title} - مسلسل من قائمة المسلسلات الرئيسية`
  }
...
const extractFilterFields = (html: string) => {
  const fields: SeriesListingResponse['filterFields'] = []
...
export const getSeriesListing = async (
  ...
  const html = await fetchHtml(`https://ak.sv/series?${params.toString()}`)
  ...
  const items = titleMatches.map((entry, index) => {
    ...
    return {
      id: `series-live-${safePage}-${index + 1}`,
      title: entry.title,
