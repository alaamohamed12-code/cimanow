...
const SHOWS_LINK_RE = /<h3[^>]*>\s*<a[^>]+href=["']([^"']*\/shows\/[^"']+)["'][^>]*>([\s\S]*?)<\/a>\s*<\/h3>/gi
...
export interface ShowsEpisodeItem {
  title: string
...
export interface ShowsDetailsPayload {
  title: string
  poster: string
  description: string
  rating: number
...
const normalizeText = (value: string): string =>
  decodeHtml(value).replace(TAG_RE, ' ').replace(SPACE_RE, ' ').trim()

const stripNoise = (html: string): string =>
  html.replace(SCRIPT_RE, ' ').replace(STYLE_RE, ' ').replace(COMMENT_RE, ' ')

const toAbsoluteUrl = (value: string): string =>
  value.startsWith('http') ? value : `https://ak.sv${value}`

const toProxyImageUrl = (value: string): string =>
  `/api/image-proxy?url=${encodeURIComponent(value.replace(/\/thumb\/\d+x\d+\//i, '/'))}`
...
const extractGenres = (html: string): string[] => {
  // Shows may use shows?category=… or series?category=…
  const genres = Array.from(
    html.matchAll(/<a[^>]+href=["'][^"']*(?:shows|series)\?category=[^"']*["'][^>]*>([\s\S]*?)<\/a>/gi)
...
const extractGallery = (html: string, mainPoster: string): string[] => {
  const posters = Array.from(new Set((html.match(POSTER_RE) || []).map((p) => toProxyImageUrl(p))))
  return posters.filter((p) => p !== mainPoster).slice(0, 12)
}

const findPosterForItem = (
  html: string,
  itemStart: number,
  itemEnd: number,
  ...
  const beforeMatches = Array.from(beforeChunk.matchAll(POSTER_RE)).map((m) => m[0])
...
const extractEpisodes = (html: string): ShowsEpisodeItem[] => {
  const items: ShowsEpisodeItem[] = []
  ...
  while ((match = EPISODE_LINK_RE.exec(html)) !== null) {
    ...
    const isEpisodeUrl = url.includes('/episode/') || url.includes('/show/episode/')
    if (!url || !label || !isEpisodeUrl || seen.has(url)) continue
    seen.add(url)
    ...
    const episodeNumRaw =
      extractFirst(label, /حلقة\s*(\d+)/i, '') ||
      extractFirst(url, /الحلقة-(\d+)/i, '0')
...
const extractActionLinks = (
  html: string
): { watchLinks: ActionLink[]; downloadLinks: ActionLink[] } => {
  const watchLinks: ActionLink[] = []
  const downloadLinks: ActionLink[] = []

  const sectionStart = html.indexOf('مشاهدة وتحميل')
  const sectionEnd =
    sectionStart >= 0 ? html.indexOf('شاهد المزيد', sectionStart) : -1
  const scopedHtml =
    sectionStart >= 0
      ? html.slice(
          sectionStart,
          sectionEnd > sectionStart ? sectionEnd : sectionStart + 12000
        )
      : html
  ...
  while ((match = ACTION_LINK_RE.exec(scopedHtml)) !== null) {
    const url = match[1]
    const label = normalizeText(match[2])
    const around = normalizeText(
      scopedHtml.slice(Math.max(0, match.index - 140), match.index + 120)
    )
...
const extractShowsRecommendations = (html: string, currentPath: string): Content[] => {
  const items: Content[] = []
  const currentId = currentPath.split('/')[2] || ''
  const matches: Array<{ url: string; title: string; start: number; end: number }> = []
  let linkMatch: RegExpExecArray | null

  while ((linkMatch = SHOWS_LINK_RE.exec(html)) !== null) {
  ...
  matches.forEach((entry, index) => {
    ...
    const chunk = html.slice(
      entry.end,
      next ? next.start : Math.min(html.length, entry.end + 380)
    )
...
export const getShowsDetails = async (pathOrUrl: string): Promise<ShowsDetailsPayload> => {
  const absoluteUrl = pathOrUrl.startsWith('http') ? pathOrUrl : toAbsoluteUrl(pathOrUrl)
  const parsed = new URL(absoluteUrl)

  if (!parsed.pathname.startsWith('/shows/')) {
    throw new Error('Only shows paths are supported')
  }
  ...
  const title =
    ...
    extractFirst(safeHtml, /<title[^>]*>([\s\S]*?)<\/title>/i, 'تفاصيل البرنامج')
  ...
  const descriptionBlock =
    between(
      cleanText,
      /مشاهدة\s*و\s*تحميل\s*(?:برنامج|مسلسل|عرض)?\s*/i,
      /فريق\s*العمل|الحلقات|المزيد|شاهد\s*المزيد|التعليقات|وسوم\s*:/i
    ) ||
    between(
      cleanText,
      /قصة\s*(?:البرنامج|المسلسل)?\s*/i,
      /فريق\s*العمل|الحلقات|المزيد|شاهد\s*المزيد|التعليقات|وسوم\s*:/i
    )
  ...
  const language =
    extractFirst(
      cleanText,
      /اللغة\s*:\s*([\s\S]{1,40}?)(?=\s+الترجمة\s*:|\s+الجودة\s*:|\s+انتاج\s*:|\s+السنة\s*:|\s+مدة\s*|$)/i
    ) || 'غير محدد'

  const quality =
    extractFirst(
      cleanText,
      /الجودة\s*:\s*([\s\S]{1,60}?)(?=\s+انتاج\s*:|\s+السنة\s*:|\s+مدة\s*|$)/i
    ) || 'غير محدد'

  const duration =
    extractFirst(cleanText, /مدة\s*(?:البرنامج|المسلسل|الحلقة)?\s*:\s*(\d+\s*(?:دقيقة|ساعة|ساعات))/i) ||
    extractFirst(
      cleanText,
      /مدة\s*(?:البرنامج|المسلسل|الحلقة)?\s*:\s*([\s\S]{1,30}?)(?=\s+اللغة\s*:|\s+الجودة\s*:|\s+انتاج\s*:|\s+السنة\s*:|$)/i,
      'غير محدد'
    )
  ...
  const country =
    extractFirst(
      cleanText,
      /انتاج\s*:\s*([\s\S]{1,40}?)(?=\s+السنة\s*:|\s+مدة\s*|\s+اللغة\s*:|\s+الجودة\s*:|$)/i
    ) || 'غير محدد'
  ...
  const { watchLinks, downloadLinks } = hasEpisodes
    ? { watchLinks: [], downloadLinks: [] }
    : extractActionLinks(safeHtml)
  const recommendations = extractShowsRecommendations(safeHtml, parsed.pathname)
