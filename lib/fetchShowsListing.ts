...
const SHOW_LINK_RE = /<h3[^>]*>\s*<a[^>]+href=["']([^"']*\/shows\/[^"']+)["'][^>]*>([\s\S]*?)<\/a>\s*<\/h3>/gi
const POSTER_RE = /https?:\/\/img\.downet\.net\/(?:thumb\/\d+x\d+\/)?uploads\/[^"'\s<>()]+/gi
const PAGE_RE = /(?:https?:\/\/ak\.sv)?\/shows\?[^"'\s>]*page=(\d+)/gi
...
export interface ShowsListingResponse {
  items: Content[]
  page: number
  totalPages: number
  filterFields: Array<{
...
const extractDescription = (title: string, metadata: string): string => {
  ...
  if (!compact) {
    return `${title} - برنامج من قائمة التلفزيون الرئيسية`
  }
...
const extractFilterFields = (html: string) => {
  const fields: ShowsListingResponse['filterFields'] = []
...
export const getShowsListing = async (
  page: number,
  sourceFilters: SourceFilters = {},
  search?: string
): Promise<ShowsListingResponse> => {
  ...
  const html = await fetchHtml(`https://ak.sv/shows?${params.toString()}`)

  const titleMatches: Array<{ url: string; title: string; start: number; end: number }> = []
  let linkMatch: RegExpExecArray | null

  while ((linkMatch = SHOW_LINK_RE.exec(html)) !== null) {
